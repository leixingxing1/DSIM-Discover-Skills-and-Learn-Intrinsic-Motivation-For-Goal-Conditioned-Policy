...
def open_experiment(dataset):
    ...
    sample_dataset = torch.from_numpy(dataset).float()
    state_size = sample_dataset.ndim
    ...
    return model, optimizer, sample_dataset,state_size,config, args, exp_dir
...
while num_frames < args.frames:
    ...
    for i in range(len(exps.obs)):
        for j in range(args.num_frames_per_proc):
            mask = 1 if j == logs1["num_frames_per_episode"] else float(exps.mask[i][j])
    ...
    vae_model, optim, dataset, state_size,config, args, save_dir = open_experiment(replay_memory)
