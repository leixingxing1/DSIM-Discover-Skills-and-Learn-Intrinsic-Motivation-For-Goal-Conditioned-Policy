...
parser.add_argument('--output_dir',default='/temp', type=str)
...
for skill_idx in range(agent.skill_n):
    ...
    q_phi_goal_loss= KLdivergence(goal_encoder.parameters(),vq_vae.encoder.parameters()).mean()* goal_state_wight 
    ...
    optimization_trajectory_actions = actions
    state = start_state
    goal = q_phi_goal
    ...
    exp_prefix = 'train/%s/task/' % (args.env,)

    with log_utils.setup_logger(exp_prefix=exp_prefix, log_base_dir=args.output_dir):
        algo.train()
